...
class UsersController extends Controller
{
    ...
    public function create()
    {
        //
        if (Auth::user()->id != 1) {
            return redirect()->route('users.index')->with('error', 'คุณไม่มีสิทธิในการแก้ไข');
        }
        else{
    ...
    public function update(User $user, UpdateUserRequest $request)
    {
        ...
        return redirect()->route('users.index')->with('success', 'แก้ไขข้อมูล [' . implode(',', array_keys($diff)) . '] เป็น [' . implode(',', $newData) . '] สำเร็จ');
    }
    ...
    {
        $user->delete();
        return redirect()->route('users.index')->with('success', 'ลบ user ใหม่สำเร็จ');
    }
}
